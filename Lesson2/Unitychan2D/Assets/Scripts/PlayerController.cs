...
public class PlayerController : MonoBehaviour
{
    ...
    void Update()
    {
		if (!m_isDamaged)
		{
            ...
            Move(x,jump);
		} else if (m_nextReleaseTime < Time.time && m_isGround)
        {
            m_isDamaged = false;
    ...
    void FixedUpdate()
    {
        ...
        Vector2 groundArea =
            new Vector2(m_boxcollier2D.size.x * 0.49f, 0.05f);
    ...
    void OnTriggerStay2D(Collider2D other)
    {
        if (other.tag == "DamageObject" && !m_isDamaged)
        {
            ...
            SendMessage("OnDamage",3,SendMessageOptions.DontRequireReceiver);
        }
    }
